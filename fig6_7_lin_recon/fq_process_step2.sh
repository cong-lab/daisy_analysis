...
SAMPLELIST=./step2_samplelist_20200916.txt
SEED=$(awk "NR==$SLURM_ARRAY_TASK_ID" $SAMPLELIST)
moltable=$(echo "$SEED" | cut -d$',' -f1)
out_fp=$(echo "$SEED" | cut -d$',' -f2)
output_dir=$(echo "$SEED" | cut -d$',' -f3)
UMI_thresh=$(echo "$SEED" | cut -d$',' -f4)

python3.6 process_fq_step2_nwh.py $moltable $out_fp $output_dir $UMI_thresh
