...
SAMPLELIST=./step7_lg_call.txt
SEED=$(awk "NR==$SLURM_ARRAY_TASK_ID" $SAMPLELIST)
in_mt=$(echo "$SEED" | cut -d$',' -f1)
out_at=$(echo "$SEED" | cut -d$',' -f2)
out_dir=$(echo "$SEED" | cut -d$',' -f3)

echo $in_mt


python3.6 /home/nwhughes/Cassiopeia/cassiopeia/ProcessingPipeline/process/lineageGroup.py $in_mt $out_at $out_dir
