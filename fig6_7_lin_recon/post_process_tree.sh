...
SAMPLELIST=./post_process_tree_sbmit_v2022Mar26.csv
SEED=$(awk "NR==$SLURM_ARRAY_TASK_ID" $SAMPLELIST)
pkl_fp=$(echo "$SEED" | cut -d$',' -f1)
char_fp=$(echo "$SEED" | cut -d$',' -f2)
out_fp=$(echo "$SEED" | cut -d$',' -f3)
...
python3.6 /home/nwhughes/Cassiopeia/cassiopeia/TreeSolver/post_process_tree.py $pkl_fp $char_fp $out_fp
