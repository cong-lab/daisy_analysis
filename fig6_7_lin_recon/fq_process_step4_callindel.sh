...
SAMPLELIST=./step4_callindelList.txt
SEED=$(awk "NR==$SLURM_ARRAY_TASK_ID" $SAMPLELIST)
alignment=$(echo "$SEED" | cut -d$',' -f1)
ref=$(echo "$SEED" | cut -d$',' -f2)
output=$(echo "$SEED" | cut -d$',' -f3)

echo $ref
echo $queries
echo $output

python3.6 process_call_indels.py $alignment $ref $output
